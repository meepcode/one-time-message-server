...
describe('POST /message with valid parameters', () => {
    ...
    it('returns a randomly generated token', async () => {
        const res = await request(app).post('/message').type('json').send({
            name: 'AJ B.',
    ...
    it('can be repeated without error', async () => {
        const res1 = await request(app).post('/message').type('json').send({
            name: 'AJ B.',
            email: 'email@example.com',
            message: 'Hello World!',
        })
        ...
        expect(res1.body.token).toMatch(
            /^([0-9a-f]){8}-([0-9a-f]){4}-([0-9a-f]){4}-([0-9a-f]){4}-([0-9a-f]){12}$/
        )
        expect(res1.status).toEqual(200)

        const res2 = await request(app).post('/message').type('json').send({
            name: 'John D.',
        ...
        expect(res2.body.token).toMatch(
            /^([0-9a-f]){8}-([0-9a-f]){4}-([0-9a-f]){4}-([0-9a-f]){4}-([0-9a-f]){12}$/
        )
    ...
    it('saves the name, message, and email to the server', async () => {
        ...
        try {
            ...
            const { name, email, message } = await client.db()
                .collection('messages')
                .findOne()
...
describe('POST /message with invalid parameters', () => {
    ...
    it('fails when missing email', async () => {
        ...
        expect(
            checkThrowsErrors(res, [
                'Email address required for leaving a message',
            ])
        )
    })
    ...
    it('fails when message too long', async () => {
        ...
        expect(
            checkThrowsErrors(res, ['Message must be at most 250 characters'])
        )
    })
})
