...
describe('POST /message then GET /message/:token', () => {
    it('retrieves the request', async () => {
        const postRes = await request(app).post('/message').type('json').send({
            name: 'AJ B.',
        ...
        const getRes = await request(app).get('/message/' + token)
    ...
    it('fails after waiting 24 hours', async () => {
        ...
        setTimeout(
            async () => {
                const res = await request(app).get('/message/' + token)
